...
buttonForm.addEventListener('click', function() {
    ...
    if (value % 2 === 0 && value <= 10 && value >= 4) {
        ...
        alert('Вы ввели некорректные данные. Для увеличения количества карт введите четное число от 4 до 10.');
        result = 4 ** 2;
        value = 4;
        resizeContainer();
    }
...
container.onclick = function(event) {
    ...
    if (pair.length === 2 && firstCard.dataset.number === secondCard.dataset.number) {
        pair.splice(0, 2);
        arrayWin.push(firstCard, secondCard);
    }

    if (pair.length === 3 && firstCard.dataset.number !== secondCard.dataset.number) {
        pair.splice(0, 2);
